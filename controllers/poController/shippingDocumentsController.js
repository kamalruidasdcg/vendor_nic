...
const { RIC, CDO } = require("../../lib/depertmentMaster");
...
const shippingDocuments = async (req, res) => {
  try {
    ...
    // const isDOcheck = await poDelingOfficersDetails(payload.purchasing_doc_no, tokenData.vendor_code);

    // if (tokenData.department_id !== RIC || tokenData.department_id !== CDO || !isDOcheck.isDO) {
    //   return resSend(res, false, 401, "You don't have permission. Please contact your Sr.", null, null);
    // }
    ...
    if (response.affectedRows) {
      ...
      resSend(
        ...
        true,
        200,
        "Shipping documents inserted successfully !",
        null,
        null
      );
    } else {
...
const List = async (req, res) => {
  try {
    ...
    const isDOcheck = await poDelingOfficersDetails(req.query.poNo, tokenData.vendor_code);

    if (tokenData.department_id !== RIC || tokenData.department_id !== CDO || tokenData.user_type !== USER_TYPE_VENDOR || !isDOcheck.isDO) {
      return resSend(res, false, 401, "You don't have permission. Please contact your Sr.", null, null);
    }

    const insp_call_query = `SELECT shipping_documents.*
