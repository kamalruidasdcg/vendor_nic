...
const {
  ...
  POfileFilterOrderBy,
  paymentAdviceFiles,
} = require("../../lib/fetchFileDirectory");
...
const download = async (req, res) => {
  ...
  if (!typeArr.includes(type)) {
    return resSend(
      ...
      false,
      400,
      "Please send valid type ! i.e. drawing, sdbg, qap",
      null,
      null
    );
  }
  ...
  if (!fileFoundQuery) {
    return resSend(
      ...
      false,
      404,
      "file not found or invalid query",
      fileFoundQuery,
      null
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (file.success && file?.data?.length) {
        ...
        const response = [
          { full_file_path: directoryPath, file_name: fileName, file_path },
        ];
      ...
      if (files.success && files?.data?.length) {
        ...
        if (payload.poNo) {
          ...
          const result = await getQuery({
            query: `SELECT lifnr AS vendor_code FROM ekko WHERE ebeln = $1`,
            values: [payload.poNo],
          });
        ...
        const resustFiles = result.map((ele) => {
          const realFileName = files?.data?.find((el) =>
            el.includes(ele.documentNo)
          );
        ...
        resSend(
          ...
          true,
          200,
          Message.FILE_FETCH_SUCCESSFULLY,
          { resustFiles },
          null
        );
      } else {
