...
exports.wdc = async (req, res) => {
  ...
  try {
    ...
    if (tokenData.user_type != USER_TYPE_VENDOR) {
      ...
      let line_item_array = await getQuery({
      ...
      obj.file_inspection_note_ref_no = payloadFiles[
        "file_inspection_note_ref_no"
      ]
        ? (invoice_filename =
            payloadFiles["file_inspection_note_ref_no"][0]?.filename)
        : null;

      obj.file_hinderence_report_cerified_by_berth = payloadFiles[
        "file_hinderence_report_cerified_by_berth"
      ]
        ? (invoice_filename =
            payloadFiles["file_hinderence_report_cerified_by_berth"][0]
              ?.filename)
        : null;

      obj.file_attendance_report = payloadFiles["file_attendance_report"]
        ? (invoice_filename =
            payloadFiles["file_attendance_report"][0]?.filename)
        : null;
...
async function handelMail(tokenData, payload, event) {
  try {
    ...
    if (payload.status == SUBMITTED) {
      // QA NODAL OFFICERS
      emailUserDetailsQuery = getUserDetailsQuery(
        "wdc_certifing_authrity",
        " $1"
      );
      emailUserDetails = await getQuery({
        query: emailUserDetailsQuery,
        values: [parseInt(payload.assigned_to)],
      });
      await sendMail(
        WDC_UPLOADING,
        dataObj,
...
exports.list = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      let line_item_array = await poolQuery({
        client,
        query: line_item_array_q,
      ...
      await Promise.all(
        line_item_array.map(async (els) => {
          ...
          let total_amount_result = await poolQuery({
            ...
            values: [req.query.poNo, els.line_item_no],
          });
          total_amount_result =
          ...
          let rest_amount =
            parseInt(els.target_amount) - parseInt(total_amount_result);
      ...
      let modfResult = get_data_result.map((el) => {
        ...
        if (line_item && Array.isArray(line_item)) {
          line_item2 = line_item.map((el2) => {
            const DOObj = line_item_array2.find(
              (elms) => elms.line_item_no == el2.line_item_no
            );
...
const getLineItemArray = async (poNo) => {
  ...
  const rest_amount =
    parseInt(target_amount_result) - parseInt(total_amount_result);
