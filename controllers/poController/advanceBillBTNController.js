...
const submitAdvanceBillHybrid = async (req, res) => {
  try {
    ...
    try {
      ...
      // PAYLOAD DATA
      const btnPayload = await advBillHybridbtnPayload(
        payload,
        "advance-bill-hybrid"
      );
      // GENERATE QUERY, DATA AND SAVE
      const btnQuery = generateQuery(
        INSERT,
        "btn_advance_bill_hybrid",
        btnPayload
      );
      const result1 = await poolQuery({
        client,
        query: btnQuery.q,
        values: btnQuery.val,
      });
  ...
  try {
    ...
    if (results && results.length) {
      results = results.map((el) => ({
        ...el,
        c_sdbg_sub_date: dateToEpochTime(el.c_sdbg_sub_date),
      }));
    }
    ...
    if (results && results.length && data && data.length) {
      results = results.map((el) => ({
        ...el,
        c_sdbg_filename: data[0].file_name,
        c_sdbg_file_path: data[0].file_path,
      }));
    }
...
const submitAdvBillBTNByDO = async (req, res) => {
  try {
    ...
    try {
      ...
      let checkBTNR = await checkBTNRegistered(
        payload.btn_num,
        "btn_advance_bill_hybrid_do",
        client
      );
      if (checkBTNR) {
        return resSend(
...
const getAdvBillHybridDataForDO = async (req, res) => {
  try {
    ...
    try {
      ...
      // CONTRACTUAL SUBMISSION DATA
      const contDateSetup = await contractualSubmissionDate(
        payload.poNo,
        client
      );
      console.log("contDateSetup", contDateSetup);
