      ...
      for (let i = 0; i < emails.length; i++) {
        if (emails[i]["email_to"]) {
          ...
          try {
            ...
            await Promise.allSettled([query({ query: `DELETE FROM ${EMAILS} WHERE id = $1`, values: [emails[i]["id"]], }),
            archiveEmails({ ...emails[i], status: STATUS_SUCCESS, remarks: JSON.stringify(email_response) })]);
            // await query({
            ...
            if (emails[i]["retry_count"] == MAIL_SEND_MAX_RETRY_COUNT) {
              await Promise.allSettled([archiveEmails({ ...emails[i], status: FAILED, remarks: error.message }), query({
          ...
          await Promise.allSettled([archiveEmails({ ...emails[i], status: FAILED, remarks: "No recipients defined" }),
          query({ query: `DELETE FROM ${EMAILS}  WHERE id = $1 `, values: [emails[i].id] })]);
        }
