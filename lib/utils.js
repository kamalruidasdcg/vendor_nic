...
async function generateQueryForMultipleData(array, tableName, pk) {
    ...
    columnArrWithOutPK.forEach((u_key, j) => {
        updateKeys = updateKeys.concat(` ${u_key} = VALUE (${u_key})`)
...
module.exports = { formatDate, formatTime, generateQuery, generateQueryArray, getEpochTime, queryArrayTOString, generateQueryForMultipleData, generateInsertUpdateQuery }