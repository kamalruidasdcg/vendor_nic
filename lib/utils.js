...
function generateQuery(action, tableName, payload, whereCondition = '') {
    const updateFields = Object.keys(payload).map(key => `${key} = ?`);
    const updateValues = Object.values(payload);
    let q = "";


    if (!action || !tableName || !payload) throw Error("please send valid parameters !!!");
...
module.exports = { generateQuery, generateQueryArray, getEpochTime, queryArrayTOString, getDateTime, formatDate, formatTime,}
