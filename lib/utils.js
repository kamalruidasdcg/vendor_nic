...
const generateInsertQuery = (action, tableName, dataObject, conditonDataObj = {}) => {
  ...
  let updateValues = Object.values(dataObject);
  let values = ""
  updateValues.forEach((el, i) => {
      if(typeof el == 'string') {
          values += `'${el}'`
      } else {
          values += `${el}`
      }
      if( i < updateValues.length -1) {
  ...
  q = `INSERT INTO ${tableName} (${insertFields.join(", ")}) VALUES (${values})`;
...
const getCreatedArr = async (Query, user_type) => {
  ...
  const Arr = data.map((item1) => ({...item1, created_at:new Date(item1.created_at).getTime()}));
...
async function generalErrorLog(error, source_name) {
  try {
    ...
    console.log("generalErrorLog function error", error.message);
  }
}
