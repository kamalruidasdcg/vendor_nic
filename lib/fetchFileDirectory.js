...
const POfileFilter = async (id) => {
  try {
    ...
    const uniqueIds = [...new Set(files.map(file => parseInt(file.split('-')[0])))];
    ...
    const filteredFiles = filterId.map(id => {
      const filteredFilesById = files.filter(file => parseInt(file.split('-')[0]) === id);
      ...
      const incrementNumbers = filteredFilesById.map(file => parseInt(file.split('-')[1]));
